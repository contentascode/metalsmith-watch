...
function buildFiles(metalsmith, paths, livereload, options, previousFilesMap) {
  ...
  async.each(
    ...
    err => {
      ...
      const nbOfFiles = Object.keys(files).length
      options.log(
        color.gray(`- Updating ${nbOfFiles} file${nbOfFiles > 1 ? "s" : ""}...`)
      )
...
function buildPattern(
  ...
  unyield(metalsmith.read())((err, files) => {
    ...
    const filesToUpdate = {}
    multimatch(Object.keys(files), patterns).forEach(
      path => (filesToUpdate[path] = files[path])
    )
...
export default function(options) {
  ...
  const plugin = function metalsmithWatch(files, metalsmith, cb) {
    ...
    Object.keys(options.paths).map(pattern => {
      ...
      const watchPatternRelative = relativePath(
      ...
      function watcherReady(err, watcher) {
        ...
        const update = () => {
          ...
          if (
            ...
            const filesToInvalidate = Object.keys(patterns).reduce(
              (acc, pattern) => {
                return [
                  ...acc,
                  ...multimatch(
            ...
            if (filesToInvalidate.length) {
              options.log(
                color.gray(
                  `- Deleting cache for ${filesToInvalidate.length} entries...`
                )
              )
          ...
          const patternsToUpdate = Object.keys(patterns).filter(
            pattern => patterns[pattern] === true
          )
          const filesToUpdate = multimatch(pathsToUpdate, patternsToUpdate).map(
            file => {
