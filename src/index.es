import { relative as relativePath, resolve as resolvePath, isAbsolute as isAbsolutePath } from 'path';
  ...
  Object.keys(collections).forEach(key => {
    for (let i = 0; i < collections[key].length; i++) {
      if (filenames.indexOf(collections[key][i].filename) > -1) {
        collections[key] = [...collections[key].slice(0, i), ...collections[key].slice(i + 1)];
        i--;
      }
    }
  });
      ...
      function watcherReady(err, watcher) {
        ...
        const update = () => {
          ...
          if (
            ...
            const filesToInvalidate = Object.keys(patterns).reduce((acc, pattern) => {
              return [
                ...acc,
                ...multimatch(Object.keys(require.cache), `${resolvePath(metalsmith._directory)}/${pattern}`)
              ];
            }, []);
